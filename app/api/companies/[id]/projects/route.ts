  ...
  const projects = db
    .prepare(
      `SELECT p.*,
       ...
       FROM projects p WHERE p.company_id = ? ORDER BY p.is_active DESC, p.name`
    )
    .all(id);
...
export async function POST(
  ...
  const result = db
    .prepare(
      "INSERT INTO projects (company_id, name, start_date, end_date) VALUES (?, ?, ?, ?)"
    )
    .run(id, name.trim(), start_date || null, end_date || null);
