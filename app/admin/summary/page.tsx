...
interface ProjectSummary { project_name: string; company_name: string; company_color: string; total_man_days: number; }
...
export default function SummaryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setPersonTotals(data.personTotals);
    setProjectData(data.projectData);
    setCompanyTotals(data.companyTotals);
    setProjectMerged(data.projectMerged);
    setActiveTab("전체");
  }, [year, month]);
  ...
  const dailyCompanies = useMemo(() => companyTotals.filter((c) => c.company_name.includes("일당")), [companyTotals]);
  const contractCompanies = useMemo(() => companyTotals.filter((c) => !c.company_name.includes("일당")), [companyTotals]);
  ...
  const downloadSheet = async (sheetData: any[][], sheetName: string, fileName: string, cols: { wch: number }[]) => {
  ...
  const downloadSection3 = async () => {
    const s: any[][] = [["업체", "현장(공사)", "총 공수"]];
    for (const p of projectData) s.push([p.company_name, p.project_name, p.total_man_days]);
    s.push(["합계", "", projectData.reduce((sum, p) => sum + p.total_man_days, 0)]);
    await downloadSheet(s, "업체별 현장별 공수", `업체별_현장별_공수_${filePrefix}.xlsx`, [{ wch: 15 }, { wch: 20 }, { wch: 10 }]);
  };

  const downloadSection4 = async () => {
    const s: any[][] = [["구분", "업체", "총 공수"]];
    for (const c of contractCompanies) s.push(["공사", c.company_name, c.total_man_days]);
    if (contractCompanies.length > 0) s.push(["공사 소계", "", contractCompanies.reduce((sum, c) => sum + c.total_man_days, 0)]);
    s.push([]);
    for (const c of dailyCompanies) s.push(["일당", c.company_name, c.total_man_days]);
    if (dailyCompanies.length > 0) s.push(["일당 소계", "", dailyCompanies.reduce((sum, c) => sum + c.total_man_days, 0)]);
    s.push([]);
    s.push(["총합계", "", companyTotals.reduce((sum, c) => sum + c.total_man_days, 0)]);
  ...
  const downloadAll = async () => {
    ...
    for (const [, data] of Object.entries(personMap)) {
      s1.push([data.name, ...days.map((d) => data.days[d] || ""), data.total]);
    }
    ...
    ws1["!cols"] = [{ wch: 10 }, ...days.map(() => ({ wch: 5 })), { wch: 8 }];
    ...
    const s3: any[][] = [["업체", "현장(공사)", "총 공수"]];
    for (const p of projectData) s3.push([p.company_name, p.project_name, p.total_man_days]);
    s3.push(["합계", "", projectData.reduce((s, p) => s + p.total_man_days, 0)]);
    const ws3 = XLSX.utils.aoa_to_sheet(s3);
    ws3["!cols"] = [{ wch: 15 }, { wch: 20 }, { wch: 10 }];

    const s4: any[][] = [["구분", "업체", "총 공수"]];
    for (const c of contractCompanies) s4.push(["공사", c.company_name, c.total_man_days]);
    if (contractCompanies.length > 0) s4.push(["공사 소계", "", contractCompanies.reduce((s, c) => s + c.total_man_days, 0)]);
    s4.push([]);
    for (const c of dailyCompanies) s4.push(["일당", c.company_name, c.total_man_days]);
    if (dailyCompanies.length > 0) s4.push(["일당 소계", "", dailyCompanies.reduce((s, c) => s + c.total_man_days, 0)]);
    s4.push([]);
    s4.push(["총합계", "", companyTotals.reduce((s, c) => s + c.total_man_days, 0)]);
    const ws4 = XLSX.utils.aoa_to_sheet(s4);
    ws4["!cols"] = [{ wch: 10 }, { wch: 15 }, { wch: 10 }];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={downloadAll}
            className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition cursor-pointer flex items-center gap-1.5">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filteredProjects.length > 0 && (
              <tr className="bg-slate-50 font-bold border-t-2 border-slate-300">
                <td className="px-5 py-3 text-slate-700" colSpan={2}>{activeTab === "전체" ? "총합계" : `${activeTab} 합계`}</td>
                <td className="px-5 py-3 text-center text-blue-600">{filteredProjects.reduce((s, p) => s + p.total_man_days, 0).toFixed(1)}</td>
              </tr>
            )}
            {filteredProjects.length === 0 && (<tr><td colSpan={3} className="px-5 py-8 text-center text-slate-400">데이터가 없습니다</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden mb-6">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {contractCompanies.map((c, i) => (
              <tr key={`c-${i}`} className="border-b border-slate-100">
                {i === 0 && <td className="px-5 py-3 font-medium text-blue-700" rowSpan={contractCompanies.length}>공사</td>}
                ...
                <td className="px-5 py-3 text-center font-bold text-blue-600">{c.total_man_days.toFixed(1)}</td>
              </tr>
            ))}
            {contractCompanies.length > 0 && (
              <tr className="bg-blue-50/50 border-b border-slate-200">
                ...
                <td className="px-5 py-2 text-center font-bold text-blue-600 text-xs">{contractCompanies.reduce((s, c) => s + c.total_man_days, 0).toFixed(1)}</td>
              </tr>
            )}
            {dailyCompanies.map((c, i) => (
              <tr key={`d-${i}`} className="border-b border-slate-100">
                {i === 0 && <td className="px-5 py-3 font-medium text-orange-700" rowSpan={dailyCompanies.length}>일당</td>}
                <td className="px-5 py-3"><span className="text-xs px-2 py-1 rounded text-white font-medium" style={{ backgroundColor: c.company_color }}>{c.company_name}</span></td>
                <td className="px-5 py-3 text-center font-bold text-orange-600">{c.total_man_days.toFixed(1)}</td>
              </tr>
            ))}
            {dailyCompanies.length > 0 && (
              <tr className="bg-orange-50/50 border-b border-slate-200">
                ...
                <td className="px-5 py-2 text-center font-bold text-orange-600 text-xs">{dailyCompanies.reduce((s, c) => s + c.total_man_days, 0).toFixed(1)}</td>
              </tr>
            )}
            {companyTotals.length > 0 && (
              <tr className="bg-slate-50 font-bold border-t-2 border-slate-300">
                <td className="px-5 py-3 text-slate-700" colSpan={2}>총합계</td>
                <td className="px-5 py-3 text-center text-blue-600">{companyTotals.reduce((s, c) => s + c.total_man_days, 0).toFixed(1)}</td>
              </tr>
            )}
            {companyTotals.length === 0 && (<tr><td colSpan={3} className="px-5 py-8 text-center text-slate-400">데이터가 없습니다</td></tr>)}
          </tbody>
        </table>
      </div>
